...
use App\Http\Controllers\Providers\ABController;
...
use App\Http\Controllers\Providers\SCRController;
    ...
    public function index(Request $request)
    {
        ...
        */
        ...
        try 
        {
            ...
            if($gameId == Providers::SBO) //Seamless
            {
                $response = SBOController::login();
            }
            else if($gameId == Providers::EVO) //seamless
            {
            ...
            else if($gameId == Providers::SA) //Seamless
            {
                $response = SAController::getGame($isMobile);
            }
            else if($gameId == Providers::Joker)//Seamless (slot need to display with game list)
            {
                $response = JokerController::openGame($request); 
            }
            else if($gameId == Providers::XE88) //Transfer (slot need to display with game list)
            {
                $response = XE88Controller::getGame($type);
            }
            else if($gameId == Providers::NOE)//Transfer (APPS)
            {
                $response = NOEController::launchGames();
            ...
            else if($gameId == Providers::PUSSY)//Transfer (APPS)
            {
                $response = PUSSYController::launchGames();
            ...
            else if($gameId == Providers::MEGA)//Transfer (APPS)
            {
                $response = MEGAController::launchGames();
            ...
            else if($gameId == Providers::AB)//Transfer
            {
                $response = ABController::getGame($isMobile);
            }
            else if($gameId == Providers::SCR)//Transfer (APPS)
            {
                $response = SCRController::launchGames();
            ...
            else
            {
                ...
                if($gameId == Providers::MEGA)
                    return view('downloads')->with(['iframe' => $iframe,
                                                'loginId' => $loginId,
                                                'ft_password' => ''
                                            ]);

                if($gameId == Providers::NOE || $gameId == Providers::PUSSY || $gameId == Providers::SCR)
                    return view('downloads')->with(['iframe' => $iframe,
                                                'loginId' => $loginId,
